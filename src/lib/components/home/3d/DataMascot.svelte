<script lang="ts">
  ...
  const vertexShader = `
    ...
    void main() {
      ...
      // DISPLACEMENT
      // Large low-frequency blob shape
      float noise = snoise(position * 0.6 + uTime * 0.3); 
      
      // Apply displacement along normal (inflate/deflate)
      // Base radius (1.0) + Noise intensity (0.4)
      vec3 newPos = position + normal * noise * 0.4;

      // Pass "elevation" relative to surface for coloring
      vElevation = noise;
