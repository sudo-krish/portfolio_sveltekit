<script lang="ts">
  ...
  const deepColor = new Color(modelMaterials.github3D.dark.deep);
  const surfColor = new Color(modelMaterials.github3D.dark.surf);
  const foamColor = new Color(modelMaterials.github3D.dark.foam);
  ...
  $: if ($theme === "light") {
    deepColor.set(modelMaterials.github3D.light.deep);
    surfColor.set(modelMaterials.github3D.light.surf);
    foamColor.set(modelMaterials.github3D.light.foam);
    uniforms.uOpacity.value = 1.0;
    customMaterial.needsUpdate = true;
  } else {
    deepColor.set(modelMaterials.github3D.dark.deep);
    surfColor.set(modelMaterials.github3D.dark.surf);
    foamColor.set(modelMaterials.github3D.dark.foam);
