<script lang="ts">
  ...
  const deepColor = new Color(modelMaterials.credentialShield.dark.deep);
  const surfColor = new Color(modelMaterials.credentialShield.dark.surf);
  const foamColor = new Color(modelMaterials.credentialShield.dark.foam);
  ...
  $: if ($theme === "light") {
    deepColor.set(modelMaterials.credentialShield.light.deep);
    surfColor.set(modelMaterials.credentialShield.light.surf);
    foamColor.set(modelMaterials.credentialShield.light.foam);
    uniforms.uOpacity.value = 1.0;
    customMaterial.needsUpdate = true;
  } else {
    deepColor.set(modelMaterials.credentialShield.dark.deep);
    surfColor.set(modelMaterials.credentialShield.dark.surf);
    foamColor.set(modelMaterials.credentialShield.dark.foam);
