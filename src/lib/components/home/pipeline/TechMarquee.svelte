  ...
  const techStack: TechItem[] = [
    {
      name: "Kafka",
      color: "#ffffff",
      logoUrl: "https://cdn.simpleicons.org/apachekafka/ffffff",
    },
    {
      name: "Debezium",
      color: "#000000",
      logoUrl: "https://cdn.simpleicons.org/databricks/ffffff",
    }, // Placeholder logo for Debezium
    {
      name: "Airflow",
      color: "#017CEE",
...
<div class="w-full overflow-hidden relative py-10 mask-gradient">
  ...
  <div
    class="absolute top-1/2 left-0 w-full h-px bg-gradient-to-r from-transparent via-border to-transparent -translate-y-1/2 opacity-20"
  ></div>
  ...
  <div class="marquee-track flex gap-6 w-max px-4">
    {#each [...techStack, ...techStack] as tech}
      ...
      >
        ...
        <div
          class="absolute inset-0 rounded-2xl bg-[var(--brand-color)] opacity-0 group-hover:opacity-10 blur-xl transition-opacity duration-500"
        ></div>

        <!-- Reflection Shine -->
        <div
          class="absolute inset-0 rounded-2xl bg-gradient-to-br from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"
        ></div>

        <!-- The Logo -->
        <div
          class="relative w-10 h-10 md:w-12 md:h-12 transition-transform duration-500 group-hover:scale-110 group-hover:drop-shadow-[0_0_10px_var(--brand-color)]"
        >
          <img
            src={tech.logoUrl}
            alt={tech.name}
        ...
        <div
          class="absolute -bottom-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 pointer-events-none"
        >
          <span
            class="text-[10px] font-mono tracking-widest uppercase text-[var(--brand-color)] font-bold bg-background/80 backdrop-blur-sm px-2 py-1 rounded-full border border-[var(--brand-color)]/20 shadow-sm whitespace-nowrap"
          >
            {tech.name}
          </span>
        </div>
