...
<script lang="ts">
  ...
  onMount(() => {
    ...
    .from(".mascot-wrapper", { opacity: 0, scale: 0.8, duration: 1.2, ease: "back.out(1.2)" }, "-=1.0");
  });
</script>
...
<section class="hero-wrapper" bind:this={heroSection} id="hero">
  ...
  -->
  <div class="container relative z-10 w-full max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center justify-center lg:justify-between flex-grow gap-10 lg:gap-0 pb-16 lg:pb-0">
    ...
    <div class="mascot-wrapper w-full lg:w-1/2 h-[40vh] lg:h-[75vh] relative flex items-center justify-center">
      <Canvas>
        <T.PerspectiveCamera makeDefault position={[5, 2, 8]} fov={35}>
        ...
        <ContactShadows opacity={0.35} scale={10} blur={2.5} far={4} color="#000000" />
      </Canvas>
    </div>
