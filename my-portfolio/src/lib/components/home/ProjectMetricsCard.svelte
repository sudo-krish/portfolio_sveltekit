<!-- src/lib/components/home/ProjectMetricsCard.svelte -->
<script lang="ts">
  import { Terminal, Activity, Database, Clock, Layers } from 'lucide-svelte';
  import { onMount } from 'svelte';
  
  let typing = false;
  
  const metrics = [
    { cmd: 'events', value: '50M+', label: '/day', icon: Activity, color: 'primary' },
    { cmd: 'warehouse', value: '<1s', label: 'query', icon: Database, color: 'accent' },
    { cmd: 'refresh', value: '5min', label: 'SLA', icon: Clock, color: 'success' },
    { cmd: 'pipelines', value: '99.9%', label: 'uptime', icon: Layers, color: 'highlight' }
  ];
...
<div class="terminal-card">
  ...
  <div class="terminal-body">
    <div class="prompt-line">
      ...
      <span class="command">./scale_metrics.sh</span>
    </div>
  ...
  <!-- Share Footer -->
  <div class="share-bar">
    <span class="share-text">Real-time at scale</span>
    <span class="share-tag">#DataEng</span>
  </div>
</div>

<style>
  /* Same styles as ImpactMetricsCard - keeping it consistent */
  .terminal-card {
  ...
  @keyframes pulse {
    ...
    50% { opacity: 0.7; box-shadow: 0 0 0 3px hsl(var(--primary) / 0); }
  }
